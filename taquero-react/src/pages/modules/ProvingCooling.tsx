...
import { Plus, CheckCircle2, Loader2, Snowflake } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Progress } from '@/components/ui/progress'
import { useCoolingMethodStore } from '@/store/provingCoolingStore'
import { Toast, ToastContainer } from '@/components/ui/toast'
import { NewCoolingMethodWizard } from '@/components/proving-cooling/NewCoolingMethodWizard'
import { AddCoolingBatchWizard } from '@/components/proving-cooling/AddCoolingBatchWizard'
import { CoolingMethod } from '@/types'
...
export function ProvingCooling() {
  const { methods, isLoading, fetchFromGoogleSheets } = useCoolingMethodStore()
  ...
  const [selectedMethod, setSelectedMethod] = useState<CoolingMethod | null>(null)
  ...
  const handleMethodCreated = () => {
    showToast('Cooling method created with Batch 1! Record 2 more batches to prove it works.', 'success')
  }

  const handleAddBatch = (method: CoolingMethod) => {
  ...
  const handleBatchAdded = (isProven: boolean) => {
    if (isProven) {
      showToast('🎉 Cooling method proven! You have completed 3 successful batches.', 'success')
    } else {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>Your Cooling Methods</CardTitle>
              {methods.length >= 100 && (
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <p className="text-lg font-medium">No cooling methods created yet</p>
              <p className="text-sm mt-2">
                Click "Create New Cooling Method" to validate your first cooling method
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-semibold text-lg">{method.foodItem}</h3>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 flex items-center gap-2 flex-wrap">
                              ...
                              <span className="font-mono">{batch.startTemp}°C → {batch.secondTempCheck}°C → {batch.thirdTempCheck}°C</span>
                              <span className="text-muted-foreground">•</span>
